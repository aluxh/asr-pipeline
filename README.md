...
This is one of the capstone projects of the Natural Language Processing Nanodegree from Udacity, where I built an deep neural network that functions as part of an end-to-end automated speech recognition (ASR) pipeline. 

![ASR Pipeline][image1]

For this project, [LibriSpeech dataset](http://www.openslr.org/12/) is used to train and evaluate the models built. Udacity has provide the algorithm to convert raw audio to feature representation in Spectrogram. And, in the notebook, you will find several neural network architecture designs that have been built and experiment mapping these audio features to transcribed text.
